...
const STORAGE_KEY = "vtp_over_speeding_report_saved_filter_v1";
...
export default function OverSpeedingReportPage({ menuKey = "" }) {
  ...
  const applyFilters = (nextStatus = "") => {
    ...
    const nextRows = OVERSPEED_EVENTS.filter((row) => {
  ...
  const handleExport = (type) => {
    ...
    const headers = ["Date", "Time", "Vehicle Reg#", "Status", "Address", "Speed", "Speed Limit", "Limit Exceeded"];
    const rows = appliedRows.map((row) => [row.date, row.time, row.object, row.status, row.address, row.speed, row.speedLimit, row.limitExceeded]);

    if (type === "CSV") {
      const content = [headers.join(","), ...rows.map((row) => row.map((cell) => `"${cell}"`).join(","))].join("\n");
      downloadFile("over-speeding-report.csv", content, "text/csv;charset=utf-8");
      setStatusText("CSV export started.");
      return;
    }
    if (type === "XLS") {
      const content = [headers.join("\t"), ...rows.map((row) => row.join("\t"))].join("\n");
      downloadFile("over-speeding-report.xls", content, "application/vnd.ms-excel");
    ...
    printWindow.document.write(`<html><head><title>Over Speeding Report</title><style>body{font-family:Arial,sans-serif;padding:18px;color:#111}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #8f9aa5;padding:4px 6px;text-align:left;vertical-align:top}th{background:#d4d9de}</style></head><body><h1>Over Speeding Report</h1><p>${rangeLabel}</p><table><thead><tr>${headers.map((header) => `<th>${header}</th>`).join("")}</tr></thead><tbody>${bodyRows}</tbody></table></body></html>`);
  ...
  if (!ready && menuKey) {
    return (
      <main className={styles.page}>
        <AccessGuardState mode="loading" title="Over Speeding Report access is loading" message="Checking your menu rights before opening this module." />
      </main>
    );
  }
  ...
  if (menuKey && !canView) {
    return (
      <main className={styles.page}>
        <AccessGuardState title="Over Speeding Report access denied" message={`You do not currently have view access for ${menuKey}.`} />
      </main>
    );
  }
              ...
              <>
                        ...
                        {appliedRows.length ? (
                          appliedRows.map((row) => (
                            <tr key={row.id}>
                              ...
                              <td>{row.time}</td>
                              <td>{row.object}</td>
                              <td className={`${styles.statusCell} ${styles.statusRunning}`}>{row.status}</td>
                              <td>{row.address}</td>
                              <td className={styles.numericCell}><span className={`${styles.metric} ${styles.metricred} ${styles.metricBold}`}>{row.speed}</span></td>
                              <td className={styles.numericCell}>{row.speedLimit}</td>
                              <td className={styles.numericCell}><span className={`${styles.metric} ${styles.metricorange} ${styles.metricBold}`}>{row.limitExceeded}</span></td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <td colSpan={8} className={styles.noDataRow}>No Records Found</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                ...
                <div className={styles.reportFooter}>
                  <div className={styles.reportFooterLeft}>
                    <button type="button" className={styles.reportToolButton} onClick={() => applyFilters("Over speeding report refreshed.")} aria-label="Refresh result"><FaSyncAlt size={14} /></button>
                  ...
                  <div className={styles.reportFooterCenter}>
                    ...
                    <select className={styles.reportScopeSelect} value={reportQuickScope} onChange={(event) => setReportQuickScope(event.target.value)}>
                      ...
                      <option value="Vehicle Reg#">Vehicle Reg#</option>
                  ...
                  <table className={styles.summaryTable}>
                    <thead>
                      <tr>
                        ...
                        <th><span className={styles.headerLabel}><span>Vehicle Reg#</span></span></th>
                        ...
                        <th><span className={styles.headerLabel}><span>Speed Limit</span></span></th>
                        <th><span className={styles.headerLabel}><span>Limit Exceeded</span></span></th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr><td colSpan={8} className={styles.noDataRow}>No Records Found</td></tr>
                      <tr><td colSpan={8} className={styles.gridFillCell}><div className={styles.gridFillArea} /></td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
