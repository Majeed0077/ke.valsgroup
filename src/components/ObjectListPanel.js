...
const FILTERS = [
  ...
  { key: "running", label: "Running", className: styles.tabRunning },
  { key: "stopped", label: "Stopped", className: styles.tabStopped },
      ...
      <div className={styles.tabs}>
        {FILTERS.map((filter) => (
          ...
          >
            ...
            <span className={styles.tabCount}>{counts?.[filter.key] ?? 0}</span>
          </button>
        ))}
      </div>
        ...
        {vehicles.length === 0 ? (
          ...
          vehicles.map((vehicle) => {
            ...
            return (
              ...
              >
                <span className={`${styles.dot} ${visualStatus.dot}`} />
