...
const nextOutputDirName =
  process.env.NEXT_OUTPUT_DIR || (shouldUseDefaultNextDir ? ".next" : ".next-dev");
...
function removePathIfPresent(targetPath) {
  try {
    ...
    console.warn(`[dev-recovery] Cleanup skipped for ${targetPath}: ${error.message}`);
  }
}
...
function runNextDev(attempt = 1) {
  ...
  child.on("exit", (code, signal) => {
    ...
    const shouldRetry =
      code !== 0 && attempt < MAX_START_ATTEMPTS && isTraceLockError(combinedOutput);

    if (shouldRetry) {
      console.warn(
        `[dev-recovery] Detected locked ${nextOutputDirName} trace file. Cleaning build output and retrying once...`
      );
      fullNextCleanup();
      runNextDev(attempt + 1);
      return;
    }
